...
base get_real_q(int n,int k){
    return q[n].eval((base)k);
}

base get_p(int n,int k){

    ll cp=(s/( (1ll)<<(n-2) ));

    base ret=0;
    for(int i=0;i<p[n-1].a.size();i++){
        base pom=p[n-1].a[i];
        ret+=pom*(get_real_q(i,cp%mod)-get_real_q(i,2*k-1));
    }
...
int main(){
    ...
    base rez=0;
    ll cp=(s/( (1ll)<<(n-1) ));
    for(int i=0;i<p[n].a.size();i++){
        base pom=p[n].a[i];
        rez+=pom*get_real_q(i,cp%mod);
    }
    printf("%d\n",rez);
