...
void adv(polyn &u,polyn &r,int n,int sz){
    ...
    polyn A1,B1,C1;
    A1=( u0-one-(((eru*u0).mod_xk(n)*u0).mod_xk(n)*u0).mul_xk(1).mod_xk(n) + er.mul_xk(1).mod_xk(n) ).mod_xk(n);
    B1=(r0-(((eru*u0).mod_xk(n)*u0).mod_xk(n)*r0).mul_xk(1).mod_xk(n)- 2*(eru*u0).mul_xk(1).mod_xk(n) ).mod_xk(n);
    C1=(ru-r0-((eru*u0).mod_xk(sz)*u0).mul_xk(1).mod_xk(sz)+ er.mul_xk(1) ).shift_left(n).mod_xk(n);

    polyn A2,B2,C2;
    A2=( er.mul_xk(1).mod_xk(n)-(eru*u0).mul_xk(1).mod_xk(n) ).mod_xk(n);
    B2=(u0-(eru*r0).mul_xk(1).mod_xk(n)+ u0-one ).mod_xk(n);
    C2=(u0*u0-u0-eru.mul_xk(1).mod_xk(sz)+ er.mul_xk(1) ).shift_left(n).mod_xk(n);


    polyn E=((mod-1)*(C1*A1.inv(n)).mod_xk(n) );
    polyn F=((mod-1)*(B1*A1.inv(n)).mod_xk(n) );

    polyn u1=((mod-1)* ((C2+A2*E).mod_xk(n)*(A2*F+B2).inv(n)).mod_xk(n) );
    polyn r1=(E+F*u1).mod_xk(n);

    for(int i=0;i<n;i++)u.pb(u1[i]);
    for(int i=0;i<n;i++)r.pb(r1[i]);
