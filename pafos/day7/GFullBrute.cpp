...
pair<pii,int>intersect(pii state,int d,pii path){
    ...
    if(dist.ff==0){/// intersect
        ...
        if(d==0)pom=get_closer_points(dist.ss.ff,dist.ss.ss,d);
            ...
            else pom=get_closer_points(state.ff,dist.ss.ss,d);
        }
...
vector<int> get_rez(pii path,int d){
...
vector<int>intersect_vectors(vector<int>a,vector<int>b){
    for(int i=1;i<=tn;i++)a[i]=max(a[i],b[i]);
    return a;
}
        ...
        for(int i=0;i<q;i++){
           ...
           // printf("%d AAA\n",i);
            if(pom.tip==1){
                ...
                paths.erase(paths.find({pat.ff,pat.ss}) );
            }
            else{
                ...
                int inf=1e9;
                vector<int>dists(tn+1,-inf);
                for(auto it=paths.begin();it!=paths.end();it++){
                    pii pp=(*it);
                    dists=intersect_vectors(dists,get_rez(pp,pom.d));
                    /*if(e){
                        printf("%d %d JEBENI\n",pp.ff,pp.ss);
                    }*/
                }
