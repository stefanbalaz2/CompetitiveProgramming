...
int get_single_pref(int idx,int d,int n){
    ...
    ret+=query_tree(root[idx][sz-d-1],0,n,0,d);
    ///printf("|%d|",query_tree(root[idx][sz-d-1],0,n,0,d));
...
void go_dp(int x,int prv,int n){
    ...
    if(mx!=-1){
       ...
       /* int vll=-1;
        vll=vect[x][0];
        printf("BEFORE PREKK %d -> %d\n",x,vll);
        for(int i=0;i<dp[idx].size();i++){
            printf("%d %d %d \n",dp[idx][i],dp_pref[idx][dp[idx].size()-i-1],get_single_pref(idx,dp[idx].size()-i-1,n ));
        }
        printf("\n");*/
    ...
    if(mx!=-1){
        ...
        for(int i=(int)tpref[idx].size()-mx-1;i<dp[idx].size();i++){
            ...
            ///printf("%d  USAO77\n",x);
            ///fflush(stdout);
            update_tree(root[idx][i],0,n,sz-i-1,dp[idx][i]);
           /// printf("%d  USAO66\n",x);
           /// fflush(stdout);
            ///printf("%d %d UBACIO\n",sz-i-1,dp[idx][i]);
       ...
       /// printf("%d  USAO88\n",x);
       /// fflush(stdout);
        update_tree(root[idx][tpref[idx].size()-1],0,n,0,dp[idx].back());
       /// printf("%d  USAO99\n",x);
       /// fflush(stdout);
       /// printf("%d %d UPDAT TREE\n",0,dp[idx].back());
    }

   /// printf("%d  USAO44\n",x);
   /// fflush(stdout);

   /// printf("GOTOV %d\n",query_tree(root[idx].back(),0,n,2,2));
...
void go(vector<int>nodes,vector<int>qind){
    ...
    for(int i=0;i<=nodes.size();i++){
        ...
        root[i].clear();
        if(i<nodes.size()){
            node_add[0][nodes[i]].clear();
            node_add[1][nodes[i]].clear();
            node_add[2][nodes[i]].clear();
        }
    }
    ...
    printf("\n");
    printf("QUERIS\n");
    for(int i=0;i<qind.size();i++)printf("%d ",qind[i]);
    printf("\n");*/

    int c=get_centroid(nodes);
    int id=-1;
    for(int i=0;i<nodes.size();i++)
        if(nodes[i]==c)id=i;
    swap(nodes[0],nodes[id]);
    prek_base_nodes(nodes);
    prek_hld_nodes(nodes);
    int cnt=prek_color_nodes(nodes);


    vector<int>rq;
    vector<vector<int>>pq(cnt);
    for(int i=0;i<qind.size();i++){
        int id=qind[i];
        int lc=lca(real_queries[id].ff.ff,real_queries[id].ff.ss);

        if(lc==c)rq.pb(id);
        else{
            pq[ncolor[lc]].pb(id);

            int td=real_queries[id].ss-(level[lc]-level[c]);

            node_add[2][c].pb({td,{id,1} });
            node_add[2][vect[c][ncolor[lc]]].pb({td-1,{id,-1} });
    ...
    dpcnt=0;
    ///printf("USAO\n");
    go_dp(nodes[0],0,nodes.size());
    ///printf("IZASAO\n");
    for(int i=0;i<nodes.size();i++)prek_pos[nodes[i]]=0;
    vector<vector<int>>next_nodes(cnt);
    for(int i=1;i<nodes.size();i++){
        int id=nodes[i];
        next_nodes[ncolor[id]].pb(id);
    }

    ///return;
    for(int i=0;i<vect[nodes[0]].size();i++){
        if(next_nodes[i].size()==0)continue;
        go(next_nodes[i],pq[i]);
    }
